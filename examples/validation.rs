...
#[session(Name, Value)]
type AuthenticationC = Send<
    S,
    Password,
    Tautology<Name, Value>,
    Constant<Name, Value>,
    Branch<S, Tautology<Name, Value>, Constant<Name, Value>, AuthenticationC2>,
>;
...
enum AuthenticationC2 {
    ...
    Failure(
        Failure,
        Branch<S, Tautology<Name, Value>, Constant<Name, Value>, AuthenticationC>,
    ),
}

#[session(Name, Value)]
type AuthenticationS = Receive<
    C,
    Password,
    Tautology<Name, Value>,
    Constant<Name, Value>,
    Select<C, LTnVar<Value, 'x', 'y'>, Incr<'x', 1>, AuthenticationS2>,
>;
...
enum AuthenticationS2 {
    ...
    Failure(
        Failure,
        Select<C, LTnVar<Value, 'x', 'y'>, Incr<'x', 1>, AuthenticationS>,
    ),
}
...
async fn s(role: &mut S) -> Result<(), Box<dyn Error>> {
    ...
    try_session(role, map, |s: AuthenticationS<'_, _>| async {
        let mut i = 0;
...
async fn c(role: &mut C) -> Result<(), Box<dyn Error>> {
    try_session(role, HashMap::new(), |s: AuthenticationC<'_, _>| async {
        ...
        loop {
            ...
            match s.branch().await? {
                AuthenticationC2::Failure(_, s2) => s = s2,
